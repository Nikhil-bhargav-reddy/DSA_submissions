class Solution:
    def maxDepth(self, root: Optional[TreeNode]) -> int:
        ...
        if not root: # if we are  at leaf nodes left or right
            return 0
    
        left = self.maxDepth(root.left) # calls first recurison, then within recurison, then within etc
        right = self.maxDepth(root.right) # calls first recursion, then within recursion etc
