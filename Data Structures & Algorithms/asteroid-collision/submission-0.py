    ...
    def asteroidCollision(self, asteroids: List[int]) -> List[int]:
        ...
        while l<len(asteroids):
            ...
            if asteroids[l] >=0: # if positive we add to stack
                ...
                if not res:
                    res.append(asteroids[l]) # append first negative value
                if res[-1] >=0: # if stack top is positive and incoming is -ve
                    while res and -asteroids[l] > res[-1]:
                        res.pop()
                    if res[-1] == -asteroids[l]: # when both equal we pop and skip to next l
                        res.pop()
                elif res[-1] <0: # if stack top is negative, we add our final -ve
                    res.append(asteroids[l])
