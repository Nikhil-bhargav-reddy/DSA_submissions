...
class Solution:
    def minMeetingRooms(self, intervals: List[Interval]) -> int:
        ...
        for i in intervals:
            if end_times and end_times[0] < i[0]: # min of our heap( min end time of meeting) finishes before  new meeting start
                heapq.heappop(end_times) # pop the smallest
                heapq.heappush(end_times,i[1]) # push the new end time
            else:
